...
/**
 * NHC TCV -> per-storm JSON writer (thin by default).
 * Outputs: active/storms/ALnnYYYY/tcv.json
 * Caches zone geometries: /js/data/zones/cache/{ZONE}.json
 *
 * CLI:
 *   php tcv_writer.php --storm=AL052025 [--file=/abs/path/MIATCVAT5_sample.txt] [--embed=0|1]
 *
 * HTTP:
 *   /active/api/tcv_writer.php?storm=AL052025[&file=/abs/path/sample.txt][&embed=0|1]
 */
...
if (!is_file($zonesCatalogPath)) fail("Missing zones catalog: {$zonesCatalogPath}");
$zonesCatalog=json_decode((string)file_get_contents($zonesCatalogPath),true);
if (!is_array($zonesCatalog)) fail('zones catalog invalid JSON');
...
if ($devFile) {
  if (!is_file($devFile)) fail("Dev file not found: {$devFile}");
  $tcvRaw=(string)file_get_contents($devFile); $sourceUrl=$devFile;
} else {
  $tcvUrl="https://www.nhc.noaa.gov/ftp/pub/forecasts/public/MIATCVEP{$tcvIdx}";
  $sourceUrl=$tcvUrl;
  $ch=curl_init($tcvUrl);
  ...
  if ($tcvRaw===false || $http!==200) {
    $empty=['meta'=>[
      ...
      'productId'=>null,'productCode'=>"MIATCVEP{$tcvIdx}",
    ...
    ok(['ok'=>true,'wrote'=>$tcvOutPath,'note'=>'TCV unavailable — wrote empty']);
  }
}
...
$tmp=$tcvOutPath.'.tmp'; file_put_contents($tmp, json_encode($out, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES)); rename($tmp,$tcvOutPath);
ok(['ok'=>true,'wrote'=>$tcvOutPath,'events'=>count($events),'features'=>count($features),'embed'=>$embed]);
