...
1. Complete: `active/index.html` no longer contains the duplicate static skip link; the single navigation-owned skip link and every script-owned storm DOM hook remain.
...
The next task is the remaining Phase 5 deterministic Active-page coverage slice. Starting from the current committed checkpoint, add immutable, non-production Atlantic and Eastern Pacific current-storm/advisory/map fixtures plus a bounded test router/harness. Do not edit `active/cache/nhc_current_storms.json`, generated tropical-map packages, retained ignored output, or production/runtime state. Exercise AL, EP, and CP through the Active page for issued and not-issued warning/surge states, partial/unavailable products, exact-ID/advisory rejection, map and warning popups, desktop/mobile layout, console, and network behavior. Keep static/automated, fixture/runtime/API, controlled-browser, owner, external-source, and deployment evidence separate.
