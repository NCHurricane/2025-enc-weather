// =======================
// County Page Builder - countyApp.js
// Builds the front-end UI for counties' weather page.
//
// For Single-zone counties, set up in countyData.js
//
...
// Injected county-specific API (set by per-county wrapper)
// Expected shape:
// {
...
async function renderCurrent() {
  ...
  if (heatEl) {
    if (showHI) {
      const val = feelsType === 'heatIndex' && feelsValue != null ? feelsValue : cur.heatIndex;
  ...
  if (chillEl) {
    if (showWC) {
      const val = feelsType === 'windChill' && feelsValue != null ? feelsValue : cur.windChill;
