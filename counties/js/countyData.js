...
export async function getCurrentConditions() {
  try {
    ...
    const primaryStationId = config.primaryStation;
    let chosen = null;

    if (primaryStationId) {
      const primary = results.find(
        (r) => r.stationId === primaryStationId && r.isFresh && r.data.temperature != null
      );
      if (primary) {
        chosen = primary;
    ...
    if (!chosen) {
      ...
      chosen =
        fresh || results.filter((r) => r.data.temperature != null).sort((a, b) => a.ageMinutes - b.ageMinutes)[0] || results[0];
    }
