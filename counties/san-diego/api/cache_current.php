...
function use_native_ca($ch): void {
...
function degToCompass(float $deg): string {
  ...
  $ix = (int) round($normalized / 22.5) % 16;
  return $dirs[$ix];
}
...
function has_display_value($value): bool {
  return $value !== null && $value !== '';
}

function fillPrimaryDisplayFields(array $zoneResult, array $zoneConfig, string $zoneName): array {
  if ($zoneName !== 'coastal') {
    return $zoneResult;
  }

  $primaryStation = $zoneConfig['primaryStation'] ?? null;
  if (!$primaryStation || !isset($zoneResult['stations'][$primaryStation])) {
    return $zoneResult;
  }
  ...
  foreach ($fallbackFields as $field) {
    if (has_display_value($zoneResult['stations'][$primaryStation]['data'][$field] ?? null)) {
      continue;
    }

    foreach ($zoneResult['stations'] as $stationId => $stationData) {
      if ($stationId === $primaryStation) {
        continue;
      }

      $fallbackValue = $stationData['data'][$field] ?? null;
      if (has_display_value($fallbackValue)) {
        $zoneResult['stations'][$primaryStation]['data'][$field] = $fallbackValue;
        error_log("Filled {$zoneName}:{$primaryStation} {$field} from {$stationId}");
        break;
      }
    }
  }

  $primaryData = $zoneResult['stations'][$primaryStation]['data'];
  $T = $primaryData['temperature'] ?? null;
  $RH = $primaryData['humidity'] ?? null;
  $HI = $primaryData['heatIndex'] ?? null;
  $WCv = $primaryData['windChill'] ?? null;
  ...
  if ($showHI) {
    $zoneResult['stations'][$primaryStation]['data']['feelsLike'] = ['type' => 'heatIndex', 'value' => $HI];
  } elseif ($showWC) {
    $zoneResult['stations'][$primaryStation]['data']['feelsLike'] = ['type' => 'windChill', 'value' => $WCv];
  }
...
try {
  ...
  foreach ($zones as $zoneName => $zoneConfig) {
    ...
    $zoneResult = fillPrimaryDisplayFields($zoneResult, $zoneConfig, $zoneName);
