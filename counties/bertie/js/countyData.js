...
async function fetchLatestObs(stationId) {
  ...
  const wspdMph = windSpeedToMph(
    p.windSpeed?.value ?? null,
    p.windSpeed?.unitCode
  );
  const wdir =
    p.windDirection?.value == null ? null : degToCompass(p.windDirection.value);
  const gustMph = windSpeedToMph(
    p.windGust?.value ?? null,
    p.windGust?.unitCode || p.windSpeed?.unitCode
  );
  ...
  let icon = p.icon || null;
  if (icon) {
    icon = icon.replace("size=medium", "size=large");
  }
  ...
  let heatIndex = heatIndexRaw,
    windChill = windChillRaw;
  if (heatIndex != null && windChill != null) {
    if (tRounded != null && tRounded >= 65) {
      windChill = null;
    } else {
      heatIndex = null;
    }
...
export async function getCurrentConditions() {
    const config = await init(); // Load config first
  const results = [];
  for (const s of STATIONS) {
    try {
      results.push(await fetchLatestObs(s.id));
    } catch (e) {
      console.warn("[current] fetch failed", s.id, e);
    }
  }
  if (results.length === 0) {
    return {
  ...
  const chosenConfig = Object.values(config.stations).find(st => st.id === chosen.stationId);
  const d = chosen.data;
  const windStr =
  ...
  const secondaries = results
    ...
    .map(r => {
      const stationConfig = Object.values(config.stations).find(st => st.id === r.stationId);
      return {
        id: r.stationId,
        name: r.stationName,
