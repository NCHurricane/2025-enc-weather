...
export async function init() {
  try {
    ...
    const currentZone = urlZone || storedZone || config.county?.defaultZone || 'mainland';
    
    // Store current zone in config for easy access
    config.currentZone = currentZone;
    
    console.log(`[countyData] Initialized for ${config.county.name} County, zone: ${currentZone}`);
    return config;
  } catch (error) {
  ...
  try {
    ...
    for (const [stationId, stationData] of stationEntries) {
      ...
      const result = {
        ...
        friendlyName: stationConfig?.friendlyName || stationData.name || stationId,
        obsTime: obsTime,
      ...
      results.push(result);
      console.log(`[current] Loaded ${stationId}: temp=${stationData.data.temperature}, age=${ageMinutes}min`);
    }
...
export async function getForecast() {
  try {
    ...
    console.log(`[countyData] Loaded ${data.periods?.length || 0} forecast periods`);
...
export async function getHourlyData() {
  try {
    ...
    const data = await response.json();
    console.log(`[countyData] Loaded ${data.properties?.periods?.length || 0} hourly periods`);
